...
var Controller = function(Layout) {
  ...
  this._lineCoordinates = function(editor, lineNo) {
    ...
    return [ 10 + lineRect.left, 10 + lineRect.top ];
  };
  ...
  this._drawPointersToActiveEditor = function() {
    ...
    context.beginPath();
    context.arc(50, self.fromPoint[1], 8, 0, 2 * Math.PI, false);
    ...
    self.state.editors[self.state.current].ranges.forEach(function(range) {
      ...
      if(coords) {
        context.beginPath();
        context.arc(self.canvasWidth/2, Math.ceil(1.01*coords[1]), 8, 0, 2 * Math.PI, false);
        context.fillStyle = 'green';
        context.fill();
        context.lineWidth = 2;
        context.strokeStyle = '#003300';
        ...
        context.moveTo(50, self.fromPoint[1]);
        context.quadraticCurveTo(50, Math.ceil(1.01*coords[1]), self.canvasWidth/2, Math.ceil(1.01*coords[1]));
