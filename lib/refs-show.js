  ...
  this._drawPointersToActiveEditor = function() {
    ...
    setTimeout(function() {
      ...
      self.state.editors[self.state.current].ranges.forEach(function(range) {
        ...
        if(coords) {
          context.beginPath();
          context.arc(Math.floor(1.1 * self.canvasWidth/2), 5 + Math.ceil(coords[1]), 8, 0, 2 * Math.PI, false);
          context.fillStyle = 'green';
          context.fill();
          context.lineWidth = 2;
          context.strokeStyle = '#003300';
          ...
          context.quadraticCurveTo(50, Math.ceil(coords[1]), Math.floor(1.1 * self.canvasWidth/2), 5 + Math.ceil(coords[1]));
