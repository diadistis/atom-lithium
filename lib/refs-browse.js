...
var Controller = function(Layout) {
  ...
  this._advanceState = function() {
    ...
    previousTransform.copyFrom(Layout.getRenderer().getTransform(self.state.editors[self.state.editors.length-1].id));

    for(var i = 0; i < self.state.editors.length; ++i) {
      var current = Layout.getRenderer().getTransform(self.state.editors[i].id);
